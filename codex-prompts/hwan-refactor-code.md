...
- Read `.claude/learnings/code/learnings.md` if exists
...
Save to: `.claude/workflow/gate-code-{YYYY-MM-DD}-{NNN}/baseline/`
...
Each saves to: `.claude/workflow/gate-code-{session}/reviews/{skill}.md`
...
- Output: `.claude/workflow/gate-code-{session}/investigations/{issue-id}.md`

## Phase 5: Synthesis with P0/P1/P2/P3

Create `.claude/workflow/gate-code-{session}/SUMMARY.md`:
...
- 전체 테스트 실행, baseline 저장: `.claude/workflow/gate-code-{session}/baseline.txt`
...
6. **Log**: `.claude/workflow/gate-code-{session}/fixes/{item-id}.md`
...
Generate new entries for `.claude/learnings/code/learnings.md` in this format:
...
- Path: `.claude/learnings/code/learnings.md`
...
❌ 실패 항목:
   - {item_id} {title}
     사유: {reason}
     로그: .claude/workflow/gate-code-{session}/fixes/{item-id}.md
     → 수동 처리 필요
...
전체 누적 학습: {total_N}개
저장 위치: .claude/learnings/code/learnings.md

다음 실행 시 자동 적용됩니다.

📁 상세: .claude/workflow/gate-code-{session}/
   - SUMMARY.md ← 먼저 보세요
   - reviews/ ← 개별 리뷰
