...
| `full` | `safe` + auto-save / auto-commit 활성화. v0.1.1부터 훅은 `main`/`master`, 위험한 파일(`.env`, 키류, `~/.claude/settings.json`), 값이 포함된 명백한 시크릿/액세스 토큰 패턴, 삭제 변경, 30개 초과 변경 시 커밋을 거부합니다. 그 모든 체크를 통과해도 여전히 `git add -A`를 실행해서 워킹 트리의 관련 없는 변경까지 스테이징할 수 있습니다. 파워 유저용; 설치 스크립트가 먼저 경고합니다. |
...
이들은 정확한 명령어와 함께 수동 단계로 안내됩니다. gstack 클론은
지원되는 환경에서 120초 제한을 둡니다. 네트워크 장애나 GitHub 접근
문제가 있으면 무한히 기다리지 않고 실패/복구 명령을 출력합니다.
...
Windows에서는 Windows Claude home을 확인하려면 `.\doctor.ps1`을 쓰세요.
WSL/Git Bash에서 `doctor.sh`를 실행하면 별도의 Linux-style home을 볼 수
있습니다. Bash로 특정 설치를 검사하려면 `CLAUDE_HOME`을 명시하세요.
...
**`--mode full`에서 추가:**
- 파일 수정 후 auto-save / auto-commit. 훅은 `main`/`master`, 위험 파일(`.env`, `*.pem`, `*.key`, `~/.claude/settings.json` 등), 값이 포함된 명백한 시크릿/액세스 토큰 패턴(API key assignment, GitHub/GitLab/Slack 토큰, private key, `sk-…`), 삭제 변경(`HWAN_AUTOSAVE_ALLOW_DELETIONS=1`로 허용), `HWAN_AUTOSAVE_MAX_FILES`(기본 30) 초과 시 커밋을 거부합니다. 통과해도 여전히 `git add -A`로 워킹 트리 전체를 스테이징해서 관련 없는 변경까지 같이 커밋할 수 있습니다. 설치 스크립트가 먼저 명시적으로 경고합니다. 끄려면 `HWAN_AUTOSAVE_DISABLE=1` 또는 `safe`/`commands-only`로 다시 설치하세요.
