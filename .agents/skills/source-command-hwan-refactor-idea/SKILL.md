---
name: "source-command-hwan-refactor-idea"
description: "PRD Gate - validate hand-written PRD before development (gstack + BMAD)"
---

# source-command-hwan-refactor-idea

Use this skill when the user asks to run the migrated source command `hwan-refactor-idea`.
...
- **gstack** installed: `~/.Codex/skills/gstack`
...
- Read `.Codex/learnings/idea/learnings.md` if exists
...
- Save copy to session: `.Codex/workflow/gate-prd-{YYYY-MM-DD}-{NNN}/prd-snapshot.md`
...
Output: `.Codex/workflow/gate-prd-{session}/bmad-prd-validate.md`
...
Each saves to: `.Codex/workflow/gate-prd-{session}/reviews/{skill}.md`
...
Create `.Codex/workflow/gate-prd-{session}/SUMMARY.md`:
...
- PRD 백업: `cp PRD.md .Codex/workflow/gate-prd-{session}/PRD.md.before-fix`
...
5. **Log**: `.Codex/workflow/gate-prd-{session}/fixes/{item-id}.md`
...
- 실제 파일 수정 안 함
- 결과를 `.Codex/workflow/gate-prd-{session}/dry-run-preview.md`에 저장
...
Generate new entries for `.Codex/learnings/idea/learnings.md` in this format:
...
- Path: `.Codex/learnings/idea/learnings.md`
...
전체 누적 학습: {total_N}개
저장 위치: .Codex/learnings/idea/learnings.md

다음 실행 시 자동 적용됩니다.

📁 상세: .Codex/workflow/gate-prd-{session}/
   - SUMMARY.md ← 먼저 보세요
...
# Notes
- **코드 절대 수정 안 함** — `.Codex/workflow/gate-prd-{session}/`에만 출력
