---
name: "source-command-hwan-refactor-git"
description: "Release Gate - final pre-deploy validation (security, QA, docs, README) using gstack + BMAD"
---

# source-command-hwan-refactor-git

Use this skill when the user asks to run the migrated source command `hwan-refactor-git`.
...
- Read `.Codex/learnings/git/learnings.md` if exists
...
Output: `.Codex/workflow/gate-release-{YYYY-MM-DD}-{NNN}/health-summary.md`
...
Each saves to: `.Codex/workflow/gate-release-{session}/audit/{skill}.md`
...
README, ARCHITECTURE, CONTRIBUTING, AGENTS.md 등 변경 사항 반영해서 자동 업데이트.
- 변경된 모든 doc 파일 식별
- diff 기반 업데이트 생성
- 사용자 확인 후 적용
...
Output: `.Codex/workflow/gate-release-{session}/docs-update.md` (변경 제안)
...
Output: `.Codex/workflow/gate-release-{session}/ship-preflight.md`
...
Create `.Codex/workflow/gate-release-{session}/SUMMARY.md`:
...
3. **문서 업데이트** (병렬):
   - `/document-release`로 README, ARCHITECTURE, AGENTS.md 등 자동 업데이트
   - 사용자 검토용 diff 생성
   - 명시적 사용자 승인 후 commit
...
Generate new entries for `.Codex/learnings/git/learnings.md` in this format:
...
- Path: `.Codex/learnings/git/learnings.md`
...
📋 PR 준비:
   - 본문 생성됨: .Codex/workflow/gate-release-{session}/pr-body.md
...
전체 누적 학습: {total_N}개
저장 위치: .Codex/learnings/git/learnings.md

다음 실행 시 자동 적용됩니다.

📁 상세: .Codex/workflow/gate-release-{session}/
