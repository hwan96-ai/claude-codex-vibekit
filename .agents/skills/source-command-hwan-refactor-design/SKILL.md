---
name: "source-command-hwan-refactor-design"
description: "Refactor-Design Gate - find UI/UX improvements after implementation (gstack + BMAD)"
---

# source-command-hwan-refactor-design

Use this skill when the user asks to run the migrated source command `hwan-refactor-design`.
...
- Read `.Codex/learnings/design/learnings.md` if exists
...
Output: `.Codex/workflow/gate-design-{YYYY-MM-DD}-{NNN}/qa-baseline.md`

`--quick` 모드면 Phase 2의 `/design-review`만 추가 실행하고 Phase 5로.

## Phase 2: Multi-perspective UI/UX parallel review

**Use Task tool to fire ALL in parallel:**
...
Output: `.Codex/workflow/gate-design-{session}/ux-improvement.md`

## Phase 4: Performance baseline

### 4a. `/benchmark` (gstack)
페이지 로드, Core Web Vitals, 리소스 크기 측정.
Output: `.Codex/workflow/gate-design-{session}/benchmark.md`
...
Create `.Codex/workflow/gate-design-{session}/SUMMARY.md`:
...
- 스크린샷 baseline 저장: `.Codex/workflow/gate-design-{session}/screenshots/before/`
...
6. **Log + screenshot**: `.Codex/workflow/gate-design-{session}/fixes/{item-id}.md` + 스크린샷
...
Generate new entries for `.Codex/learnings/design/learnings.md` in this format:
...
- Path: `.Codex/learnings/design/learnings.md`
...
📸 스크린샷 비교:
   - before/after 저장: .Codex/workflow/gate-design-{session}/screenshots/
   - 의도한 변경: {N}건 확인됨
...
전체 누적 학습: {total_N}개
저장 위치: .Codex/learnings/design/learnings.md

다음 실행 시 자동 적용됩니다.

📁 상세: .Codex/workflow/gate-design-{session}/
